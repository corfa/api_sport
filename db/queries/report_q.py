...
def get_data_for_report_noactiv_abonnement_on_departamet_in_file(session: Session, id: int):
    ...
    for employ in employees:
        ...
        if abonnement.activ == False:
            ...
            abonnements_date_start.append(str(abonnement.update_at))
    ...
    create_file_for_noactiv_exel(data_activ, "responsible")


def get_data_for_report_activ_abonnement_on_departamet_in_file(session: Session, id: int):
    employees = session.query(Employees).filter(Employees.department_id == id).all()
    names, phone_number, deportament, date_born, is_employ, abonnements_status, abonnements_cost, abonnements_date_start = [], [], [], [], [], [], [], []
...
def create_file_for_noactiv_exel(data: dict,file_name:str):
    df = pd.DataFrame({'Имя': data["names"],
                       ...
                       'дата рождения': data["date_born"],
                       'работик/родственник': data["is_employ"],
                       'дата отмены': data["abonnements_date_start"],
                       'статус абонемента': data["abonnements_status"],
                       'стоимость абонемента': data["abonnements_cost"]})
